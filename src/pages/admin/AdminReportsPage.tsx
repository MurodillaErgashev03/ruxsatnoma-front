...
export const AdminReportsPage: React.FC<AdminReportsPageProps> = () => {
  ...
  const [reportsList, setReportsList] = useState<ReportItem[]>([
    ...
    },
    {
      ...
      title: 'DЎX hududida oʻtkazilgan inspeksiya aktlari va dalolatnomalar своди',
  ...
  const handleApproveReport = (id: string) => {
    ...
    alert('Hisobot muvaffaqiyatli qabul qilindi va tasdiqlandi!');
  };
  ...
  const handleConfirmReturnReport = () => {
    ...
    alert(`Hisobot (${selectedReportForReturn.code}) DЎXga qaytarildi (Возврат)!`);
  ...
  return (
    <div className="space-y-6 font-sans pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#E4E7EA] pb-4">
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-[#2E7D4F] bg-[#F0F7F1] px-2.5 py-1 rounded border border-[#D9EBDC]">
            HISOBOTLAR SUBTIZIMI (П. 4.2.6)
          </span>
          <h1 className="text-lg md:text-xl font-bold text-[#1A1F24] mt-1.5">
            Hisobotlar va Formalar Boshqaruvi
          </h1>
          <p className="text-xs text-[#5A646D] mt-0.5">
            Hududiy oʻrmon xoʻjaliklaridan (DЎX) keladigan davriy hisobotlarni qabul qilish, tekshirish, qaytarish (возврат) va topshirish muddatlarini belgilash
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={() => alert('Barcha hisobotlar своди Excel formatida yuklab olindi!')}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-[#E4E7EA] rounded-2xl p-4 shadow-xs space-y-1">
          <span className="text-xs font-bold uppercase tracking-wider text-[#5A646D]">Qaytarilgan (Возврат)</span>
      ...
      {/* 3. Status Tabs & Search Filter Bar (п. 4.2.6.8) */}
        ...
        <div className="border-b border-[#E4E7EA] pb-2">
          <Tabs
            tabs={[
              ...
              { id: 'returned', label: 'Qaytarilgan (Возврат)', count: returnedCount },
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-[#F8F9FA] border-b border-[#E4E7EA] text-[#5A646D] font-bold uppercase tracking-wider">
              <tr>
                ...
                <th className="py-3.5 px-4 text-right">Amallar (П. 4.2.6)</th>
              </tr>
            </thead>
                  ...
                  <td className="py-3.5 px-4 whitespace-nowrap">
                    ...
                    {r.status === 'returned' && (
                      <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-[#FFFBEB] text-[#B45309] border border-[#FDE68A] inline-flex items-center gap-1" title={r.returnReason}>
                        <RotateCcw className="w-3 h-3" /> Qaytarilgan (Возврат)
                      </span>
                    )}
                    {r.status === 'overdue' && (
      ...
      {/* 7. New Report Form Modal (п. 4.2.6.1 & 4.2.6.5) */}
      {isAddFormModalOpen && (
        <Modal
          isOpen={isAddFormModalOpen}
          onClose={() => setIsAddFormModalOpen(false)}
          title="Yangi Hisobot Formasi va Topshirish Muddatini Belgilash (П. 4.2.6.5)"
          ...
          footer={
            <div className="flex items-center justify-end gap-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() => {
                  alert("Yangi hisobot shakli e'lon qilindi va DЎXlarga yuborildi!");
