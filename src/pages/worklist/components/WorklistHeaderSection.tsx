  ...
  return (
    <div className="space-y-3 font-sans">
      ...
      <nav className="text-xs text-[#5A646D] flex items-center gap-1.5 font-medium">
        <button
          type="button"
          onClick={() => onNavigate?.(isCentralAdmin ? 'manager_decision' : 'leskhoz_inbox')}
        ...
        <span className="text-[#1A1F24] font-bold">
          {isCentralAdmin ? 'Respublika arizalari reyestri' : 'Mening arizalarim (Мои заявки)'}
        </span>
      </nav>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#E4E7EA] pb-4">
        <div>
          ...
          <div className="text-xs md:text-sm text-[#5A646D] mt-1 flex flex-wrap items-center gap-2">
            <span>{isCentralAdmin ? 'Oʻrmon xoʻjaligi agentligi (Markaziy apparat)' : 'Toshkent davlat oʻrmon xoʻjaligi'}</span>
            <span>·</span>
            <span>
              {isCentralAdmin ? 'Respublika boʻyicha arizalar:' : 'Sizga biriktirilgan arizalar:'}{' '}
